...
Tippy("Timer script has started!", 2000, 1)

global suspendlight := 0
global tippyindex := 1
global numTimers := 0
global timers := []
global timermessages := []
global timerwaits := []
global timertimes := []
global timerindex := []
...
{	
	...
	Tippy(suspendlight = 1 ? "Timer Hotkeys Suspended! (light)" : "Timer Hotkeys Unsuspended! (light)", 1000, -1)
	return
}
...
$^+!\::
{
	...
	suspendlight += 1
	suspendlight := Mod(suspendlight, 2)
	Tippy(suspendlight = 1 ? "Timer Hotkeys Suspended! (full)" : "Timer Hotkeys Unsuspended! (full)", 1000, -1)
	return
}
...
$[::
{
	InputBox, Message, Message, What do you want to set a timer for?,,100,100
	if (ErrorLevel = 1 or Message = "") 
	{
		Return
	}
	InputBox, waitTime, Wait Time, How many minutes before run?,,100,100
	if (ErrorLevel = 1 or waitTime = "")
	{
		Return
	}
	if waitTime is not Integer
	{
		MsgBox, Time needs to be an integer!
		Return
	}
	fn := Func("TimerMessage").bind(Message, tippyindex)
	numTimers += 1
	timers.Push(fn)
	timermessages.Push(Message)
	timerwaits.Push(waitTime)
	timertimes.Push(A_NowUTC)
	timerindex.Push(tippyindex)
	tippyindex += 1
	SetTimer, %fn%, % waitTime * -60000
	Return
}
...
$+[::
{
	if (numTimers == 0)
	{
		MsgBox, There are no timers running!
		Return
	}
	index := 1
	temp := ""
	...
	{
		temp .= """"
		temp .= timermessages[index]
		temp .= """: "
		temp .= (timerwaits[index] - TimeSince(timertimes[index]))
		temp .= " minute(s)."
		temp .= "`r`n"
		index += 1
		if (index > 3)
	...
	InputBox, indexremove, Remove Timer, Which timer do you want to remove? (0 or blank to cancel)`r`n%temp%,,1000,%height%
	if (indexremove = "" or indexremove = 0)
	{
		Return
	}
	if indexremove is not Integer
	{
		MsgBox, Index must be an integer!
		Return
	}
	if ((indexremove > numTimers) or (indexremove < 0))
	{
		MsgBox, % (numTimers > 0) ? "Index must be between 0 and " . %numTimers% : "There are no timers running!"
		Return
	}
	fn := timers[indexremove]
	SetTimer, %fn%, Delete
	timers.RemoveAt(indexremove)
	timermessages.RemoveAt(indexremove)
	timerwaits.RemoveAt(indexremove)
	timertimes.RemoveAt(indexremove)
	Tippy("", 0, timerindex[indexremove])
	timerindex.RemoveAt(indexremove)
	numTimers -= 1
	Return
}
